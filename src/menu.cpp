...
const uint8_t Presets_Logo[] = {
    // width=30, height=30
    0xff, 0xff, 0xff, 0x3f, 0xff, 0x3f, 0xfe, 0x3f, 0xff, 0x1f, 0xfc, 0x3f, 0xff, 0x8f, 0xf8, 0x3f, 0xff, 0xc7, 0xf1, 0x3f, 0xff, 0xe3, 0xe3, 0x3f, 0xff, 0xf1, 0xc7, 0x3f, 0xff, 0xf8, 0x8f, 0x3f, 0x7f, 0xfc, 0x1f, 0x3f, 0x3f, 0xfe, 0x3f, 0x3e, 0x1f, 0xff, 0x7f, 0x3c, 0x8f, 0xff, 0xff, 0x38, 0xc7, 0xff, 0xff, 0x39, 0x83, 0xff, 0xff, 0x30, 0x83, 0xff, 0x7f, 0x20, 0x9f, 0xff, 0x7f, 0x3c, 0x9f, 0xff, 0x7f, 0x3c, 0x9f, 0xff, 0x7f, 0x3c, 0x9f, 0xff, 0x7f, 0x3c, 0x9f, 0x1f, 0x7e, 0x3c, 0x9f, 0x0f, 0x7c, 0x3c, 0x9f, 0xcf, 0x79, 0x3c, 0x9f, 0xcf, 0x79, 0x3c, 0x9f, 0xcf, 0x79, 0x3c, 0x9f, 0xcf, 0x79, 0x3c, 0x9f, 0xcf, 0x79, 0x3c, 0x3f, 0x00, 0x00, 0x3e, 0x7f, 0x00, 0x00, 0x3f, 0xff, 0xff, 0xff, 0x3f, 0xff, 0xff, 0xff, 0x3f};
...
void AddItem(const char *Name, Item_Type Type, const uint8_t *Image, xpItem item, xpPage LocalPage, xpPage nextpage, ItemFunction function)
{
    ...
    {
        // 计算新项目的位置
        item->x = LocalPage->item.tail->x + IMG_SPACE; // 使用固定间距
        item->y = Init_y;
 ...
 *
 * 本数用于根据目标值、当前值及已设定的PID参数，算出下一时刻的控制量。
 *
...
void Change_MenuState(xpMenu Menu, MenuState state)
{
...
static void Calculate_Menu_xy(xpMenu Menu, xpPage Page, xpItem now_item, xpItem next_item)
{
    ...
    case TEXT:
        ...
        if (Page->page_state == PAGE_MOVING)
        {
            if ((next_item->id - now_item->id) > 0)
                Menu->headY -= ((next_item->id - now_item->id) > (Page->length - VER_RES / Menu->text_space)) ? ((Page->length + 1 - VER_RES / Menu->text_space) * Menu->text_space) : Menu->text_space;
            else
                Menu->headY += ((now_item->id - next_item->id) > (Page->length - VER_RES / Menu->text_space)) ? ((Page->length + 1 - VER_RES / Menu->text_space) * Menu->text_space) : Menu->text_space;
    ...
    case IMAGE:
        ...
        {
            if ((next_item->id - now_item->id) > 0)
                Menu->headX -= ((next_item->id - now_item->id) > (Page->length - HOR_RES / Menu->image_space)) ? ((Page->length + 1 - HOR_RES / Menu->image_space) * Menu->image_space) : Menu->image_space;
            else
                Menu->headX += ((now_item->id - next_item->id) > (Page->length - HOR_RES / Menu->image_space)) ? ((Page->length + 1 - HOR_RES / Menu->image_space) * Menu->image_space) : Menu->image_space;
            Page->page_state = PAGE_STATIC; // 更新页面状态为静态显示
        }
...
static void Calculate_Cursor(xpMenu Menu, xpPage Page, xpItem now_item, xpItem next_item)
{
    ...
    {
        if (now_item->itemType == PARENTS)
            CursorAnimationParam_Init(Menu, next_item);
        else if (now_item->itemType == RETURN)
            CursorAnimationParam_Change(Menu);
        return;
    }
    switch (Page->type)
    {
    case TEXT:
        ...
        if (Menu->_cursor.TargrtRow > (Menu->text_space * (VER_RES / Menu->text_space) - Menu->text_space) || Menu->_cursor.TargrtRow < 0) // 防止光标溢出可视范围
        {
            if (abs(now_item->id - next_item->id) == 1)
                Menu->_cursor.TargrtRow = now_item->y - Font_Hight;
        }
        break;
    case IMAGE:
        ...
        {
            if (abs(now_item->id - next_item->id) == 1)
                Menu->_cursor.TargrtColumn = now_item->x;
        }
        break;
    default:
        break;
            ...
            if (temp_item->itemType == DATA)
            {
                ...
                case DATA_SWITCH:
                    ...
                    if (*(uint8_t *)temp_item->element->data->ptr == true)
                        OLED_DrawBox(temp_item->x + HOR_RES - 33 + 2, temp_item->Animation_y - Font_Hight + 2, Font_Hight - 4, Font_Hight - 4);
                    break;
                default:
                    break;
                }
...
{
    xpItem temp_item = Page->item.head;

    for (uint16_t i = 0; i <= Page->length; i++)
    {
        temp_item->Animation_x = PID_Animation(temp_item->x, temp_item->Animation_x, &Menu->_animation.ImagePage_X);
        if (temp_item->Animation_x >= -IMG_WIDTH && temp_item->Animation_x <= HOR_RES + IMG_WIDTH)
        {
            int draw_x = static_cast<int>(temp_item->Animation_x);
            int draw_y = 12;
            // 居中显示图标
            OLED_DrawXBMP(draw_x, draw_y, IMG_WIDTH, IMG_HEIGHT, temp_item->logo);
        }
        temp_item = temp_item->nextItem;
    }
...
/* 在此填入按键扫描程序
 * 功能：执行按键扫描，根据接收到的数确定按键方向
 * 参数：无
 * 返回值：Menu_Direction 枚举类型，表示按键的方向，没有按键按下时返回MENU_NONE
 */
Menu_Direction BtnScan(void)
{
    handleButtons();
    ButtonAction action = getButtonAction();

    // Serial.printf("Menu BtnScan: action = %d\n", action); // 添加调试信息

    // 将 ButtonAction 映射到 Menu_Direction
    switch (action)
    {
    case RIGHT_DOWN:
        return MENU_DOWN;
    ...
    case DATA:
        ...
        else
        {
            if (item->itemFunction == NULL)
                ParameterSetting_Widget(Menu);
            else
                (item->itemFunction)(Menu); // 执行项目的函数
            ...
            else
            {
                if (Menu->dir == MENU_ENTER)
                {
                    if (Menu->now_item->element->data->function != NULL && Menu->now_item->element->data->Function_Type == EXIT_EXECUTE)
...
void Menu_Loop(xpMenu Menu)
{
    ...
    // Serial.printf("Menu BtnScan: Dir = %d\n", Dir);
    if (Menu->menu_state == MENU_INIT && Dir != MENU_NONE)
    {
        Menu->menu_state = MENU_RUN;
        Serial.println("Menu->menu_state == MENU_INIT && Dir != MENU_NONE\r\n");
        // 初始化状态下，非空检查now_item并绘制当前菜单项
        if (Menu->now_item != NULL)
        {
            Serial.println("Menu->now_item != NULL\r\n");
            Draw_Menu(Menu, Menu->now_item->page.location, Menu->now_item, Menu->now_item);
        }
        else
        {
            Serial.println("Menu->now_item is NULL\r\n");
