...
U8G2_SSD1306_128X64_NONAME_F_HW_I2C u8g2(U8G2_R0, 5, 21, 19);
...
void U8g2_text(uint16_t x, uint16_t y, const char *s)
{
    u8g2.setCursor(x, y);
    u8g2.print(s);
}
...
void OLED_DrawGlyph(uint16_t x, uint16_t y, uint16_t encoding)
{
...
void OLED_DrawHLine(uint16_t x, uint16_t y, uint16_t w)
{
    u8g2.drawHLine(x, y, w);
}

void OLED_DrawDisc(uint16_t x, uint16_t y, uint16_t r)
{
    u8g2.drawDisc(x, y, r);
}

uint16_t OLED_DrawUTF8(uint16_t x, uint16_t y, const char *str)
{
